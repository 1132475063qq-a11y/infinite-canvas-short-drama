...
本项目采用按任务复杂度动态选择模型的开发方式，目标是在保证开发质量的前提下尽量节省 Codex 使用额度。

每次完成当前任务后，在最终回复末尾必须增加：

## 下一步模型建议

推荐模型：<模型名称>
下一步任务：<下一步应该做什么>
原因：<为什么这个模型足够或为什么需要升级>
更便宜模型是否可用：<可以/不建议，并说明原因>
升级条件：<什么情况下应该升级到更强模型>
完成后是否降级：<如果可以，说明应该降回哪个模型>
...
除非有明确理由，否则下一步默认推荐 Luna Max。

适用于：
- 已有明确需求和实现方案的编码
- React / TypeScript 开发
- Canvas 节点和组件开发
- 节点拖拽、连线、缩放、框选、吸附、自动对齐
- UI / CSS
- API Adapter
- 类型定义
- 普通状态管理
...
- 多模块联动
- 多文件复杂逻辑
- Store / History / Graph / Persistence 相互影响
- 中型重构
- 较难定位的 Bug
- Luna Max 连续两次尝试仍未解决的问题
...
- Canvas 核心架构
- 核心数据模型
- Agent Team 与 Canvas 通信协议
- Artifact / Topic / Message / Event 协议
- Agent / Skill 编排
- 大规模跨模块重构
- 核心性能设计
- 重要架构 Review
...
只有在以下情况下推荐：
- Sol Max / Sol Xhigh 已无法解决
- 极复杂系统级架构推演
- 最高风险核心系统最终审查

普通开发任务不得推荐 Sol Ultra。

## 成本优先级

始终遵守：

Luna Max → Terra Max → Sol Max → Sol Xhigh → Sol Ultra

只有当下一档模型无法可靠完成任务时才能升级。
...
如果高阶模型完成架构或复杂问题后，后续工作已经变成明确实现任务，必须主动提醒我切回 Luna Max。

不要仅因为任务重要就推荐高成本模型，要根据任务实际推理复杂度判断。
