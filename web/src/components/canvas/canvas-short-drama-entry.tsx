...
import { AlignLeft, ArrowRight, Bot, Check, ChevronDown, ChevronUp, CircleDot, Clapperboard, FolderKanban, MoreHorizontal, Palette, Pencil, Plus, Sparkles, Type, Upload, UserRound, X } from "lucide-react";
...
export function CanvasLinkedProjectEmptyState({
    projectName,
    hasChapter,
    onAddFirstChapter,
    onAddFilmCharacter,
            ...
            <div className="pointer-events-auto w-full max-w-[440px] rounded-lg border p-3 shadow-sm backdrop-blur" data-canvas-no-zoom style={{ background: theme.toolbar.panel, borderColor: theme.toolbar.border, color: theme.node.text }}>
                ...
                <div className="mt-3 grid grid-cols-3 gap-1.5">
                    ...
                    >
                        ...
                        <span className="truncate">创建场景</span>
                    </button>
                    <button
                        type="button"
                        onClick={onAddFilmShot}
                        className="flex h-9 min-w-0 items-center justify-center gap-1 rounded-md border px-2 text-[var(--fs-label)] font-medium"
                        style={{ borderColor: theme.node.stroke, background: theme.node.fill }}
                    >
                        <CircleDot className="size-3.5 shrink-0" />
                        <span className="truncate">创建镜头</span>
                    </button>
                    <button
                        type="button"
                        onClick={onAddFilmCharacter}
                        className="flex h-9 min-w-0 items-center justify-center gap-1 rounded-md border px-2 text-[var(--fs-label)] font-medium"
                        style={{ borderColor: theme.node.stroke, background: theme.node.fill }}
                    >
                        <UserRound className="size-3.5 shrink-0" />
                        <span className="truncate">角色资产</span>
                    </button>
                </div>
                {hasChapter ? (
                    <button type="button" onClick={onAddFirstChapter} className="mt-2 inline-flex h-7 items-center gap-1 text-[var(--fs-label)]" style={{ color: theme.node.muted }}>
