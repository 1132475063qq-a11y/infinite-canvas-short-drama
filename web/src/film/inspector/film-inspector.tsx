...
function NodeInspector({ node, project, tabs, activeTab, onTabChange, onProjectChanged, onProjectionChanged }: { node: CanvasNodeData; project?: ProjectDetail; tabs: readonly string[]; activeTab: string; onTabChange: (tab: string) => void; onProjectChanged?: () => Promise<unknown> | void; onProjectionChanged?: FilmInspectorProps["onProjectionChanged"] }) {
    ...
    const artifactScoped = node.filmKind === "acting" || node.filmKind === "prompt_pack";
...
function FilmFactEditor({ node, project, activeTab, onProjectChanged, onProjectionChanged }: { node: CanvasNodeData; project?: ProjectDetail; activeTab: string; onProjectChanged?: FilmInspectorProps["onProjectChanged"]; onProjectionChanged?: FilmInspectorProps["onProjectionChanged"] }) {
    ...
    const editable = ["scene", "shot", "character", "location", "prop", "acting", "prompt_pack"].includes(node.filmKind || "");

    useEffect(() => setDraft(buildDraft(node, project)), [node.id, node.filmKind, project]);
    ...
    const save = async () => {
        ...
        try {
            ...
            } else if ((node.filmKind === "acting" || node.filmKind === "prompt_pack") && resolved.shot) {
                const artifactType = node.filmKind === "acting" ? "acting" : "video_prompt_pack";
                const response = await saveProjectFilmArtifact(project.project.id, { shotId: resolved.shot.id, artifactType, status: "draft", payload: artifactFromDraft(node.filmKind, draft, resolved.contract) });
                const title = `${filmText(resolveFilmNode({ ...node, filmKind: "shot" }, project).contract.shotCode) || resolved.shot.title} · ${node.filmKind === "acting" ? "Acting" : "Prompt Pack"}`;
    ...
    return <>
        ...
        {activeTab === "Overview" || activeTab === "Definition" ? <PrimaryEditor kind={node.filmKind!} draft={draft} set={set} project={project} /> : <SpecializedTab kind={node.filmKind!} tab={activeTab} draft={draft} set={set} resolved={resolved} project={project} />}
...
function PrimaryEditor({ kind, draft, set, project }: { kind: FilmNodeKind; draft: Record<string, string>; set: (key: string, value: string) => void; project?: ProjectDetail }) {
...
function SpecializedTab({ kind, tab, draft, set, resolved, project }: { kind: FilmNodeKind; tab: string; draft: Record<string, string>; set: (key: string, value: string) => void; resolved: ReturnType<typeof resolveFilmNode>; project?: ProjectDetail }) {
    ...
    if (tab === "Versions") return <InspectorSection title="版本记录"><InspectorRow label="当前版本" value={resolved.artifact ? `v${resolved.artifact.objectVersion}` : `v${resolved.asset?.currentVersion?.version || resolved.asset?.character?.version || 1}`} /><InspectorRow label="版本总数" value={String(resolved.asset?.versionCount || resolved.shot?.contractVersion || 1)} /></InspectorSection>;
...
function buildDraft(node: CanvasNodeData, project?: ProjectDetail) {
    ...
    return {
        ...
        ...stringFields(contract, ["objective", "obstacle", "tactic", "beat", "performance", "continuityLocks", "sceneContext", "activeReferences", "locationMap", "firstFrame", "bodyOrientation", "gaze", "formatMode", "optics", "timedActionBeats", "physics", "lighting", "audio", "style", "positiveLocks", "localFailureLocks", "emittedNegativeConstraints", "compiledPrompt"]),
    };
}
